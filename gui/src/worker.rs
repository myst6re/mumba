...
impl WorkerLoop {
    ...
    pub fn run(&mut self) {
        ...
        for received in &self.rx {
            match received {
                ...
                Message::LaunchGame => {
                    Self::launch_game(&installation, &ffnx_installation, &steam_exe)
                }
    ...
    fn launch_game(
        ...
        if let Err(e) = match game_installation.edition {
            installation::Edition::Standard => {
                Self::launch_game_directly(&ffnx_installation.exe_path(), &ffnx_installation.path)
            }
            installation::Edition::Steam | installation::Edition::Remastered => {
                Self::launch_game_via_steam(game_installation, ffnx_installation, steam_exe)
                    .or_else(|_| {
                        Self::launch_game_directly(
    ...
    fn install_game_and_ffnx(
        ...
        let ffnx_dir = if cfg!(unix) {
            ...
            PathBuf::from(&self.env.ffnx_dir)
        };
