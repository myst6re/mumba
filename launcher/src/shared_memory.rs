...
use windows::Win32::System::Memory::{
    CreateFileMappingA, MapViewOfFile, FILE_MAP_ALL_ACCESS, MEMORY_MAPPED_VIEW_ADDRESS,
    PAGE_READWRITE,
};
    ...
    fn create_shared_memory(
        ...
        let map_view_of_file = unsafe {
            match CreateFileMappingA(
                ...
                Ok(mapping) => Some(MapViewOfFile(mapping, FILE_MAP_ALL_ACCESS, 0, 0, 0)),
                Err(e) => {
    ...
    fn read_command_from_game(&self, duration_ms: u32) -> Option<u32> {
        ...
        unsafe {
            match WaitForSingleObject(self.launcher_can, duration_ms) {
                ...
                WAIT_FAILED => {
                    error!(
                        "Error when waiting for launcher can semaphore: #{}",
                        GetLastError().0
                    );
                    return None;
                }
                e => {
                    error!(
                        "Error when waiting for launcher can semaphore: Unknown {}",
                        e.0
                    );
                    return None;
                }
