...
impl Env {
    pub fn new() -> Result<Self, std::io::Error> {
        ...
        let (cache_dir, data_dir, config_dir) = match base_dirs {
            ...
            None => (cache_fallback.clone(), data_fallback.clone(), config_fallback.clone())
        };
        let cache_dir = Self::create_dir(&cache_dir).and(Ok(cache_dir)).or_else(|_|
            Self::create_dir(&cache_fallback).and(Ok(cache_fallback))
        )?;
        let data_dir = Self::create_dir(&data_dir).and(Ok(data_dir)).or_else(|_|
            Self::create_dir(&data_fallback).and(Ok(data_fallback))
        )?;
