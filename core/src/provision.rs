...
pub fn download_zip(url: &str, local_zip_name: &str, target_dir: &PathBuf, env: &Env) -> Result<(), Error> {
    ...
    match std::fs::remove_file(&archive_path) {
        Ok(ok) => ok,
        Err(e) => warn!("Cannot remove file {}: {}", archive_path.to_string_lossy(), e)
    };
    ret
}

pub fn extract_zip(source_file: &PathBuf, target_dir: &PathBuf) -> Result<(), zip::result::ZipError> {
...
fn from_reader<R: ?Sized>(reader: &mut R, archive_path: &PathBuf, target_dir: &PathBuf) -> Result<(), Error>
where
    R: Read
{
